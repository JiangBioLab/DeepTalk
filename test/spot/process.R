...
load("../pathways.rda")
load("../geneinfo.rda")
lrpairs <- read.csv("../lrpairs_new.csv")
...
write.csv(newmeta, file = "./st_meta.csv")

st_data <- read.csv("./ad_st_new.csv",row.names = 1)
obj <- createDeepTalk(st_data = as.matrix(st_data),
                     st_meta = newmeta[, c(1,3,2)],
                     species = "Mouse",
                     if_st_is_sc = T,
                     spot_max_cell = 1,
                     celltype = newmeta$celltype)
obj <- find_lr_path(object = obj, lrpairs = lrpairs, pathways = pathways)

#obj <- dec_cci1(object = obj, 
#                celltype_sender = 'Vip',
#                celltype_receiver = 'L5.NP')

obj <- dec_cci_all1(object = obj)
...
write.csv(cell_pair_all, file = "cell_pair_all.csv", row.names = FALSE)
write.csv(obj@lr_path$lrpairs, file = "lrpairs.csv", row.names = FALSE)