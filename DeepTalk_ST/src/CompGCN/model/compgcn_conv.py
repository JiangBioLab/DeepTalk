...
class CompGCNConv(MessagePassing):
	...
	# Removing num_rels as a property
	def __init__(self, in_channels, out_channels, node_rel_comp_func):
		super(self.__class__, self).__init__()

		#self.p 			     = params
		self.in_channels	     = in_channels
		self.out_channels	     = out_channels
		self.node_rel_comp_func	 = node_rel_comp_func
		self.act 		         = torch.tanh
		self.device		         = None
		...
		self.drop		= torch.nn.Dropout(0.1) #self.p.dropout)
	...
	def rel_transform(self, ent_embed, rel_embed):
		if   self.node_rel_comp_func == 'circ_conv': trans_embed  = ccorr(ent_embed, rel_embed)
		elif self.node_rel_comp_func == 'sub': 	trans_embed  = ent_embed - rel_embed
		elif self.node_rel_comp_func == 'mult': trans_embed  = ent_embed * rel_embed
		elif self.node_rel_comp_func == 'no_rel': trans_embed  = ent_embed
	...
	def __repr__(self):
		# Commenting out since this is the only place num_rels is used
