...
class My_loss_new(nn.Module):
    ...
    def forward(self, M, F_out ,S, T):

        M_probs = torch.nn.functional.softmax(M[0,:,:], dim=1)
        F_probs = torch.sigmoid(F_out)

        M_probs_filtered = M_probs * F_probs[:, np.newaxis]

        S_filtered = S * F_probs[:, np.newaxis]

        G_pred = torch.matmul(M_probs.t(), S_filtered)
        S1 = F.cosine_similarity(G_pred, T, dim=0).mean()
        S2 = F.cosine_similarity(G_pred, T, dim=1).mean()
        '''
...
class Matchsc_st(nn.Module):
    ...
    def forward(self, data):
        ...
        self.scores = scores / self.config['descriptor_dim']

        np.random.seed(seed=2023)
        F_out = np.random.normal(0, 1, data['descriptors0'].shape[-1])
        self.F_out = torch.tensor(
            F_out, device='cuda:0', requires_grad=True, dtype=torch.float32
        )

        return self.scores,self.F_out
            

    def get_loss(self,scores,data):

        SC = data['descriptors0'].float().T[:,0,:]
        ST = data['descriptors1'].float().T[:,0,:]
        #print(self.scores.float())
        #print(self.F.float())

        loss_fun = My_loss_new()

        loss,score0,score1 = loss_fun(self.scores.float(), self.F_out.float(),SC, ST)
